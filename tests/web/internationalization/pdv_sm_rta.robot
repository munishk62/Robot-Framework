...
...                 Set suite variables ``${PDV_DATADRIVER_MENU_USER_KEY}``, ``${PDV_DATADRIVER_MENU_MODULE}``; locale from ``Configure I18n Suite Locale From Contract``; bundle root from env override ``BUNDLES_DIR`` / ``WFM_RESOURCE_BUNDLES_DIR`` or auto-discovery under ``test_data/environments/<TEST_ENVIRONMENT>/i18n_data/resource_bundles`` via ``Configure I18n Bundles Dir From Contract``. Optional contract YAML augments menu paths / DB2 sources only.
...
Test Tags           dev:kapil    action:read    i18n    i18n_sm_rta


*** Variables ***
${PDV_DATADRIVER_MENU_USER_KEY}     SM1_STORE1
${PDV_DATADRIVER_MENU_MODULE}       rta
