...
Test Tags           dev:amol    battc00087    action:write    config:rta    bat_phase1    refactoring_required_PC00134    module:timekeeping


*** Test Cases ***
BATTC00087: Verify webclock clock in/clock out transaction and display of date & time as per unit time zone
    [Documentation]    Validates clock-in and clock-out with timezone verification and exception management verification.
    ...
    ...    This test validates that a user can perform clock-in and clock-out punches in the RTA web clock.
    ...
    ...
    ...    The test will be skipped if the clock functionality is not applicable.
