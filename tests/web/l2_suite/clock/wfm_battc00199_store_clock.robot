*** Settings ***
Documentation       Verify the user is able to do all categories (Clock In, Meal In, Meal End, Break In, Break Out, Clock Out)
...                 of punch transaction for a single day (WFM-110492)
...
Test Tags           dev:amol    battc00199    config:rta    bat_phase2    module:timekeeping


*** Test Cases ***
BATTC00199: Verify the user is able to do repeating categories (Clock In, Meal In, Meal End, Break In, Break Out, Clock Out) of punch transaction for a single day
    ...
    ...
    ...    This test validates that a user can perform all punch types (clock, meal, break) repeatedly in a single day.
    ...
    ${is_meal_applicable}    Check Meal Applicability For User In DB    user_key=ESS2_STORE2
    Skip If    not ${is_meal_applicable}    Meal functionality not applicable - DB validation failed for user ESS2_STORE2

    ${is_break_applicable}    Check Break Applicability For User In DB    user_key=ESS2_STORE2
    Skip If    not ${is_break_applicable}    Break functionality not applicable - DB validation failed for user ESS2_STORE2
    ...
    ${ess_user}    Get User    user_key=ESS2_STORE2
    ...
    # Perform the 6-punch sequence twice for repeating transactions
    FOR    ${_}    IN RANGE    2
