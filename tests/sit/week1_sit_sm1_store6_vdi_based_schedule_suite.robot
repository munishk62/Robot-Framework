...
Suite Setup         Run Only Once    Pre Setup Schedule For Week 1 SM1 Store6 SIT
Test Teardown       Close Browser


*** Test Cases ***
SITTC80001 : Verify VDI Based Workload Regeneration When Schedule Is Generated And Not Edited And Unpublished
    [Documentation]    SITTC80001: Workload is generated for the store ZSITSTORE006 for VDI based activity (1FTE from 10a to 6p for day 2 to Day 6) for PW#1
    ...    Optimized schedule is generated for the store ZSITSTORE006 for PW#1. No edits are present in the schedule
    ...
    ...
    [Tags]    dev:azar    rws    sittc80001    sit_v3    sit_bp    sit    web    sit_r22_epic    schedule_dependent
    ...
    Set To Dictionary    ${vdi_import_data}    metric_id=${vdi_metric_id}    activity_name=${vdi_activity_name}
    ...
    ${vdi_metric_id}=    Get System Value    VDIMetricId    VDI_METRIC_ID_1
    ${vdi_activity_name}=    Get System Value    VDIActivity    VDI_ACTIVITY_1
    # post_no_shift_days is 0-based — matches skip_days convention in VDI file generation directly.
    Set To Dictionary    ${vdi_import_data}    metric_id=${vdi_metric_id}    activity_name=${vdi_activity_name}
    ...    skip_days=${vdi_regen_data}[post_no_shift_days]
