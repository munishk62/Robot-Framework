...
Suite Setup         Run Only Once    Pre Setup Schedule For Week 2 SM1 Store6 SIT
Test Teardown       Close Browser


*** Test Cases ***
SITTC80002 : Verify VDI Based Workload Regeneration When Schedule Is Generated And Edited And Unpublished
    [Documentation]    SITTC80002: Workload is generated for the store ZSITSTORE006 for VDI based activity (1FTE from 10a to 6p for day 2 to Day 6) for PW#2
    ...    Optimized schedule is generated for the store ZSITSTORE006 for PW#2.
    ...    Schedule is manually edited (shift added on Day 1), then VDI import with "Delete and Generate all schedules" regenerates schedule.
    ...
    ...    **Pre-Conditions (Setup):**
    ...    - Workload generated: 1FTE from 10a to 6p for Day 2-6 (skip Day 0 & 6)
    ...    - Schedule generated using optimized algorithm
    ...    - No manual edits applied to schedule
    ...    - Schedule remains unpublished
    ...
    ...    **Test Flow:**
    ...    1. Verify pre-state: Days 2-6 have 10a-6p shifts, Days 0,6 have no shifts
    ...    2. Add manual shift on Day 0 (12p-8p)
    ...    3. Logout, login as CORP, perform VDI import with 2p-10p for Days 1,3,4,5,7 (skip 2,6)
    ...    4. Verify workload regenerated: Days 1,3,4,5,7 have 8hrs, Days 2,6 have 0hrs
    ...    5. Verify schedule regenerated: Days 1,3,4,5,7 have 2p-10p shifts, Days 2,6 have no shifts
    ...    6. Manual edit is overridden by regeneration
    [Tags]    dev:azar    rws    sittc80002    sit_v3    sit_bp    sit    web    sit_r22_epic    schedule_dependent
    ...    sit_schedule_dependent
    ${vdi_regen_data}=    Get VDI Workload Regeneration Data
    ...    template_name=default
    ...    week_start_date=2_0
    ...    associate_user_key=ESS1_STORE6_SIT
    ...
    Login And Launch WFM Web App    user_key=SM1_STORE6_SIT
    # ── Step 2: Navigate to Week Schedule and verify pre-state ────────────────
    ...
    # ── Pre-state: verify schedule/demand/unallocated hours before VDI import ─
    ...
    # Days without shifts (0,5): 0hrs for all types
    ...
    # Add a shift on Day 0 from 12:00 PM to 08:00 PM (manual edit)
    # This represents a manual schedule adjustment that should be overridden by VDI regeneration
    ...
    ${smuser}=    Get User    user_key=SM1_STORE6_SIT
    ...
    # Days with shifts (1,3,4,5,7): Use post shift pattern data for hours calculation
    ...
    # Days without shifts (2,6): 0hrs for all types
