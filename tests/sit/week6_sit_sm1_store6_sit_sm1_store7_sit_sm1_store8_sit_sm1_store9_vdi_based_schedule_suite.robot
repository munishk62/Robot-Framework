...
Suite Setup         Run Keywords    Run Only Once    Pre Setup Schedule For Week 6 SM1 Store6 SIT
...                     AND    Run Only Once    Pre Setup Schedule For Week 6 SM1 Store7 SIT
...                     AND    Run Only Once    Pre Setup Schedule For Week 6 SM1 Store8 SIT
...                     AND    Run Only Once    Pre Setup Schedule For Week 6 SM1 Store9 SIT
Test Teardown       Close Browser


*** Test Cases ***
SITTC80014 : Verify VDI Based Workload Regeneration Using Delete And Generate Based On Unit Attribute Option When Schedule Is Edited And Published
    [Documentation]    SITTC80014 (WFM-133522): Same four-store unit-attribute setup for PW#6. Schedules are EDITED
    ...    (Day 1 12p-8p added) and PUBLISHED before the VDI File 2 import, which is performed with the "Delete and
    ...    Generate based on Store/unit Attribute" regeneration option selected on the RFP Upload page (same
    ...    mechanism as SITTC80001). This is the comprehensive variant.
    ...
    ...    **Expected per unit attribute (edited, published):**
    ...
    [Tags]    dev:shantanu    rws    sittc80014    sit_v3    sit_bp    sit    web    sit_r22_epic    schedule_dependent
    ...
    ${vdi_import_data}=    Get VDI Import Data    template_name=vdi_import_schedule_regeneration_sit    week_start_date=6_0
    ...
    FOR    ${store}    IN    @{stores}
        ...
        IF    ${edited}
            ...
            ...    ${edit_day1_hours}    min_hours=${vdi_regen_data}[scheduled_min_hours]
            ...    max_hours=${vdi_regen_data}[scheduled_max_hours]
            Verify Associate Demand Hours For Multiple Days On Week Schedule Page On Web
            ...    ${associate_display_name}    ${edit_day1_zero}
            Verify Associate Unfilled Hours For Multiple Days On Week Schedule Page On Web
            ...    ${associate_display_name}    ${edit_day1_zero}
        END
    ...
    FOR    ${store}    IN    @{stores}
        ...
        ELSE
            ...
            ...    ${preserved_no_shift_data}    default    6
            Verify Shift Not Exists For Associate On Multiple Days On Week Schedule Page On Web    ${associate_display_name}
            ...
            ...    ${week_prefix}    ${vdi_regen_data}[hours_zero]    6
            Verify Associate Scheduled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}
