...
...                     Run Only Once    Pre Setup Schedule For Week 4 SM1 Store6 SIT
...                     AND    Run Only Once    Pre Setup Schedule For Week 4 SM1 Store7 SIT
...                     AND    Run Only Once    Pre Setup Schedule For Week 4 SM1 Store8 SIT
...                     AND    Run Only Once    Pre Setup Schedule For Week 4 SM1 Store9 SIT
Test Teardown       Close Browser


*** Test Cases ***
SITTC80012 : Verify VDI Based Workload Regeneration Using Regenerate Based On Unit Attribute Option When Schedule Is Edited And Unpublished
    [Documentation]    SITTC80012 (WFM-133518): Same four-store unit-attribute setup for PW#4, but each schedule is
    ...    EDITED (Day 1 12p-8p added) and left unpublished before the VDI File 2 import and the "Delete and
    ...    Generate" based on unit attribute regeneration via Advanced Settings.
    ...
    ...    **Expected per unit attribute (edited):**
    ...    - ZSITSTORE006 (all) → schedule regenerated; manual Day 1 edit overridden (now 2p-10p).
    ...    - ZSITSTORE007 (unedited) → schedule PRESERVED (edited → not regenerated); Day 1 12p-8p retained.
    ...    - ZSITSTORE008 (do not) → schedule PRESERVED; Day 1 12p-8p retained.
    ...    - ZSITSTORE009 (blank) → default, schedule PRESERVED; Day 1 12p-8p retained.
    [Tags]    dev:azar    rws    sittc80012    sit_v3    sit_bp    sit    web    sit_r22_epic    schedule_dependent
    ...    sit_schedule_dependent
    ${vdi_regen_data}=    Get VDI Workload Regeneration Data
    ...    template_name=default    week_start_date=4_0
    ${post_unfilled_full_override}=    Get VDI Workload Regeneration Data
    ...    template_name=default    post_unfilled_min_hours=0.0    post_unfilled_max_hours=8.0
    ${post_unfilled_partial_override}=    Get VDI Workload Regeneration Data
    ...    template_name=default    post_unfilled_min_hours=0.0    post_unfilled_max_hours=8.0
    ${vdi_regen_overrides}=    Get VDI Workload Regeneration Data
    ...    template_name=default    driver_start_time=10:00    driver_end_time=18:00
    ${vdi_import_data}=    Get VDI Import Data    template_name=vdi_import_schedule_regeneration_sit    week_start_date=4_0
    ...
    FOR    ${store}    IN    @{stores}
        ...
        ...    ${pre_no_shift_data}    default    @{pre_no_shift_days_template}
        ...
        ${pre_shift_hours}=    Build Expected Hours Dictionary For Multiple Days On Week Schedule Page On Web
        ...    ${week_prefix}    ${pre_shift_pattern_data}    @{pre_shift_days}
        ...
        Verify Associate Scheduled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}    ${pre_zero_hours}
        Verify Associate Demand Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}    ${pre_shift_hours}
        ...
        Verify Associate Unfilled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}    ${pre_zero_hours}
        IF    ${edited}
            ...
            ${edit_day1_hours}=    Build Expected Hours Dictionary For Multiple Days On Week Schedule Page On Web
            ...    ${week_prefix}    ${vdi_regen_data}[hours_full_day]    ${day1_index}
            ${edit_day1_zero}=    Build Expected Hours Dictionary For Multiple Days On Week Schedule Page On Web
            ...    ${week_prefix}    ${vdi_regen_data}[hours_zero]    ${day1_index}
            Verify Associate Scheduled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}
            ...    ${edit_day1_hours}    min_hours=${vdi_regen_data}[scheduled_min_hours]    max_hours=${vdi_regen_data}[scheduled_max_hours]
            Verify Associate Demand Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}    ${edit_day1_zero}
            Verify Associate Unfilled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}    ${edit_day1_zero}
        END
    ...
    FOR    ${store}    IN    @{stores}
        ...
        ${vdi_schedule_regeneration_option}=    Get System Value    VDI_Schedule_Regeneration    PLEASE_SELECT
    ...
    ...    fiscal_year=${fiscal_year}    fiscal_week=${fiscal_week}    operation=${delete_generate_operation}    schedule_preference=${vdi_schedule_regeneration_option}
    ...
    FOR    ${store}    IN    @{stores}
        ...
        Select Week Number On Week Schedule Page On Web    ${week_offset}
        Go To Schedule Page On Web
        ${regenerated}=    Evaluate    '${store}[attribute]' == 'all' or ('${store}[attribute]' == 'unedited' and not ${edited})
        ...
        ...    ${week_prefix}    ${vdi_regen_data}[hours_zero]    @{post_no_shift_days_template}
        Verify Associate Demand Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}    ${demand_shift_days}
        Verify Associate Demand Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}    ${demand_zero_days}
        IF    ${regenerated}
            ...
            ${regen_no_shift_data}=    Build Employee Shift Setup Data With Overrides For Verification    ${regen_no_shift_data}    default    @{post_no_shift_days_template}
            Verify Shift Exists For Associate On Multiple Days On Week Schedule Page On Web    ${associate_display_name}
            ...
            Verify Associate Scheduled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}
            ...    ${demand_shift_days}    min_hours=${vdi_regen_data}[scheduled_min_hours]    max_hours=${vdi_regen_data}[scheduled_max_hours]
            Verify Associate Scheduled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}    ${demand_zero_days}
            Verify Associate Unfilled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}
            ...    ${demand_shift_days}    min_hours=${vdi_regen_data}[post_unfilled_min_hours]    max_hours=${vdi_regen_data}[post_unfilled_max_hours]
            Verify Associate Unfilled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}    ${demand_zero_days}
        ELSE
            ...
            ${preserved_no_shift_data}=    Build Employee Shift Setup Data With Overrides For Verification    ${preserved_no_shift_data}    default    6
            Verify Shift Not Exists For Associate On Multiple Days On Week Schedule Page On Web    ${associate_display_name}
            ...    ${preserved_no_shift_data}    ${week_offset}    ${vdi_regen_overrides}[driver_start_time]    ${vdi_regen_overrides}[driver_end_time]
            VAR     @{all_shift_with_edit}    ${day1_index}    @{pre_shift_days_template}
            ${preserved_sched_days}=    Build Expected Hours Dictionary For Multiple Days On Week Schedule Page On Web
            ...    ${week_prefix}    ${vdi_regen_data}[hours_full_day]    @{all_shift_with_edit}
            ${preserved_sched_zero}=    Build Expected Hours Dictionary For Multiple Days On Week Schedule Page On Web    ${week_prefix}    ${vdi_regen_data}[hours_zero]    6
            Verify Associate Scheduled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}
            ...    ${preserved_sched_days}    min_hours=${vdi_regen_data}[scheduled_min_hours]    max_hours=${vdi_regen_data}[scheduled_max_hours]
            Verify Associate Scheduled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}    ${preserved_sched_zero}
            ${preserved_unfilled_day1}=    Build Expected Hours Dictionary For Multiple Days On Week Schedule Page On Web
            ...
            ...    ${week_prefix}    ${vdi_regen_data}[hours_full_day]    6
            Verify Associate Unfilled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}
            ...    ${preserved_unfilled_day1}    min_hours=${post_unfilled_full_override}[post_unfilled_min_hours]
            ...    max_hours=${post_unfilled_full_override}[post_unfilled_max_hours]
            Verify Associate Unfilled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}
            ...    ${preserved_unfilled_partial}    min_hours=${post_unfilled_partial_override}[post_unfilled_min_hours]
