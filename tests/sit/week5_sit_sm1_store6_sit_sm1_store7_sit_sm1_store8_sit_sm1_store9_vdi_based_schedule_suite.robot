...
...                     Run Only Once    Pre Setup Schedule For Week 5 SM1 Store6 SIT
...                     AND    Run Only Once    Pre Setup Schedule For Week 5 SM1 Store7 SIT
...                     AND    Run Only Once    Pre Setup Schedule For Week 5 SM1 Store8 SIT
...                     AND    Run Only Once    Pre Setup Schedule For Week 5 SM1 Store9 SIT
Test Teardown       Close Browser


*** Test Cases ***
SITTC80013 : Verify VDI Based Workload Regeneration Using Regenerate Based On Unit Attribute Option When Schedule Is Not Edited And Published
    [Documentation]    SITTC80013 (WFM-133521): Same four-store unit-attribute setup for PW#5. Schedules are NOT
    ...    edited but PUBLISHED before the VDI File 2 import and the "Delete and Generate" based on unit attribute
    ...    regeneration via Advanced Settings.
    ...
    ...    **Expected per unit attribute (not edited, published):**
    ...    - ZSITSTORE006 (all) → schedule regenerated to 2p-10p (published + unedited is still regenerated).
    ...
    [Tags]    dev:azar    rws    sittc80013    sit_v3    sit_bp    sit    web    sit_r22_epic    schedule_dependent
    ...    sit_schedule_dependent
    ${vdi_regen_data}=    Get VDI Workload Regeneration Data    template_name=default     week_start_date=5_0
    ${vdi_regen_overrides}=    Get VDI Workload Regeneration Data    template_name=default    driver_start_time=10:00    driver_end_time=18:00
    ${vdi_import_data}=    Get VDI Import Data    template_name=vdi_import_schedule_regeneration_sit    week_start_date=5_0
    ...
    FOR    ${store}    IN    @{stores}
        ...
        IF    ${regenerated}
            ${regen_shift_data}=    Get Employee Shift Setup Data    template_name=${post_shift_template}    ess_user_key=${store}[associate]
        ...
        ELSE
            ...
            Verify Associate Unfilled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}    ${preserved_unfilled_full}
            Verify Associate Unfilled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}    ${preserved_unfilled_partial}
            Verify Associate Unfilled Hours For Multiple Days On Week Schedule Page On Web    ${associate_display_name}    ${demand_zero_days}
        END
        Log Out From Web Application
        Close Browser
