"""
Provider for day off request data.
"""
...
class DayOffDataProvider(BaseDataProvider):
    """Provides test data specifically for day off request tests."""

    def __init__(self):
        super().__init__("day_off")

    @keyword
    def get_day_off_data(self, test_id=None, template_name="default", **overrides):
        """
        Returns test data for a day off request test case.
        ...
        Returns:
            Dict: Test data object with all required fields for day off requests
        """
        ...
        base_data = self._resolve_constant_values(
            base_data, ["DayOffReasonType", "RequestStatus", "AccrualBalanceReason"]
        )
    ...
    def _process_date_placeholders(self, data):
        ...
        for key in ["start_date", "end_date"]:
            if key in data and isinstance(data[key], str):
                week_num, day_num = date_helper.parse_relative_week_day_notation(
                    data[key]
                )
